...
module.exports.getOrderHPDController = async (req, res) => {
...
module.exports.getOrderHPDControllerById = async (req, res) => {
    const token = req.headers["auth-token"];
    const decoded = jwt.verify(token, jwtSecret)
    ...
    try {
        const allOrder = await Order3.findOne({ItemID: ItemID}, {
            TMonth: 0,
            TYear: 0,
            TDay: 0,
