...
module.exports.getBacodjiNotificationController = async (req, res) => {
...
module.exports.deleteBacodjiNotificationController = async (req, res) => {

    try {
        const token = req.headers["auth-token"];
        const decoded = jwt.verify(token, jwtSecret)
        const deleteNotification = 
            await Notification2.deleteMany({},{}).then().catch((error) => {console.log(error)})
