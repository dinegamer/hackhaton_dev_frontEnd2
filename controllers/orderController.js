...
module.exports.getOrderControllerById = async (req, res) => {
    const token = req.headers["auth-token"];
    const decoded = jwt.verify(token, jwtSecret)
    ...
    try {
        const allOrder = decoded.site === 'bacodji' ? await Order2.findOne({ItemID: ItemID}, {
            TMonth: 0,
            TYear: 0,
            TDay: 0,
        }).then().catch(err => console.log(err)) : 
        decoded.site === 'hypodrome' ?
        await Order3.findOne({ItemID: ItemID}, {
            TMonth: 0,
            TYear: 0,
            TDay: 0,
        }).then().catch(err => console.log(err)) :
         await Order.findOne({ItemID: ItemID}, {
            TMonth: 0,
            TYear: 0,
            TDay: 0,
        }).then().catch(err => console.log(err))
...
module.exports.addOrderController = async (req, res) => {
    const token = req.headers["auth-token"];
    const decoded = jwt.verify(token, jwtSecret)
    ...
    try {
        ...
        const transaction_id = "INTEC"+ date_format.getDate() + date_format.getMonth() + date_format.getFullYear() + date_format.getHours() + date_format.getMinutes() + date_format.getSeconds()

        const newOrder = decoded.site === 'bacodji' ? await new Order2(
            {
                ...req.body.data, OrderTime: transaction_time, TMonth: month, TDay: day,
                TYear: year, OrderDate: date_format.toLocaleDateString("fr"), TransactionId: transaction_id, TransactionTime: transaction_time, TransactionDate: date_format.toLocaleDateString("fr")
            }
        ).save() 
        ...
        : decoded.site === 'hypodrome' ? await new Order3(
            {
                ...req.body.data, OrderTime: transaction_time, TMonth: month, TDay: day,
                TYear: year, OrderDate: date_format.toLocaleDateString("fr"), TransactionId: transaction_id, TransactionTime: transaction_time, TransactionDate: date_format.toLocaleDateString("fr")
            }
        ...
        const allUser = decoded.site === 'bacodji' ? 
