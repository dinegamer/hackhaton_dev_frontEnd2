...
module.exports.getHypodromeNotificationController = async (req, res) => {
...
module.exports.deleteHypodromeNotificationController = async (req, res) => {

    try {
        const token = req.headers["auth-token"];
        const decoded = jwt.verify(token, jwtSecret)
        const deleteNotification = 
            await Notification3.deleteMany({},{}).then().catch((error) => {console.log(error)})
